...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
      {/* Глобальный фон: Сетка и виньетка как на лендинге */}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="w-full max-w-md relative z-10">
        <div className="bg-black/40 border border-white/[0.08] rounded-[2rem] p-8 sm:p-10 backdrop-blur-xl shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div className="relative">
                ...
                <input type="text" placeholder="Operator Callsign (Username)" required className="w-full bg-white/[0.02] border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-500/50 transition-colors" />
              </div>
            )}
            <div className="relative">
              <Mail className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
              <input type="email" placeholder="Secure Email" required className="w-full bg-white/[0.02] border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-500/50 transition-colors" />
            </div>
            <div className="relative">
              <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
              <input type="password" placeholder="Encryption Key (Password)" required className="w-full bg-white/[0.02] border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-zinc-500/50 transition-colors" />
            </div>
